...
# Init Variables
img_path: str = "elements/placeholder.png"
content: str = ""
prob: int = 0
prediction: str = ""
...
def predict_img(model: models.Model, path_to_image: str) -> tuple[Any, str]:
    """
...
def button_pressed(state: State) -> None:
    """
    ...
    state.prediction = (
        f"WasteWise is {state.prob}% confident that this is {top_pred} waste."
    )
    state.content = state.img_path  # Make sure content is updated


# Define actions when image is uploaded
def on_change(state: State, var_name: str, new_img_path: str) -> None:
    """
